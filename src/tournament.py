...
class Tournament:
    def __init__(self, options):
        ...
        self.game = Game(options['engines'][0]['path'], options['engines'][1]['path'], options['engines'][0]['name'], options['engines'][1]['name'])
        self.game.mode = f"{self.options['controls']['mode']} {self.options['controls']['value']}"
        self.scores = [0, 0]
    ...
    def play(self):
        ...
        for i in loop:
            ...
            loop.set_description(f"{self.scores[0]}-{self.scores[1]}-{i+1-self.scores[0]-self.scores[1]}")
        ...
        print(f"ELO: {elo_difference(self.scores[1], n_draws, n_games):.2f} ± {elo_incertitude(self.scores[1], self.scores[0], n_draws):.2f}")
